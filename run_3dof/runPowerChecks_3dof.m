%%%
%%% File:           runPowerChecks_3dof.m
%%% Description:    Does some simple power checks for the Optickle SubSQL 
%%%                 reference model in the 3dof starting configuration
...
% initialize the AC differential sweep
s.pos_start = zeros(opt.Ndrive, 1);
s.pos_end = zeros(opt.Ndrive, 1);

s.start = -5e-9
s.stop = 5e-9

%% Sweep option EM 
s.pos_start(didx.nEMx) = s.start;
s.pos_start(didx.nEMy) = -s.start;

s.pos_end(didx.nEMx) = s.stop;
...
figure(1)
subplot(1,2,1)
plot( pos(didx.nEMx,:)-pos(didx.nEMy,:) , squeeze(sigDC(pidx.nACx_DC,:)),'LineWidth',2,'color','b')
hold
plot( pos(didx.nEMx,:)-pos(didx.nEMy,:) , squeeze(sigDC(pidx.nACy_DC,:)),'LineWidth',2,'color','r')
set(gca,'XLim',[2*s.start;2*s.stop]);
xlabel('AC differential tuning (m)');
ylabel('Intra cavity power (W)');
legend('ACx\_DC','ACy\_DC');
grid on;


subplot(1,2,2)
plot( pos(didx.nEMx,:)-pos(didx.nEMy,:) , squeeze(sigDC(pidx.nPDa_DC,:)),'LineWidth',2,'color','b')
set(gca,'XLim',[2*s.start;2*s.stop]);
hold
plot( pos(didx.nEMx,:)-pos(didx.nEMy,:), squeeze(sigDC(pidx.nPDs_DC,:)),'LineWidth',2,'color','g')
xlabel('AC differential tuning (m)');
ylabel('Power (W)');
legend('PDa\_DC','PDs\_DC');
grid on;
